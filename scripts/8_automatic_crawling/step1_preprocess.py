...
class TextPreprocessor:
    """Clean text for LDA topic modeling."""
...
def preprocess_for_lda(text: str) -> tuple:
    """Heavy preprocessing for LDA (removes stopwords, punctuation, etc.)"""
    try:
...
def preprocess_for_llm(text: str) -> tuple:
    """Light preprocessing for LLM classification (keeps context)."""
    try:
        pp = TextPreprocessor()
...
def write_chunks(df: pd.DataFrame, path: str, chunk_size: int = 50000) -> None:
    """Write dataframe in chunks to avoid memory issues."""
...
def main():
    ...
    # Input: extracted telegram data
    extracted_dir = '../../../../telegram_2024/usc-tg-24-us-election/extracted'
    nodes_file = f"{base_dir}/nodes_level_{level}.csv"
    ...
    # Clean up - remove empty/invalid rows
